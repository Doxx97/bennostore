            ...
            <div class="card border-0 shadow-sm h-100">
                <div class="card-header bg-white py-3 d-flex flex-wrap justify-content-between align-items-center">
                    <h6 class="m-0 fw-bold mb-2 mb-md-0">
                        <i class="bi bi-graph-up-arrow text-primary me-2"></i> Tren Penjualan
                    </h6>
                ...
                <div class="card border-0 shadow-sm bg-primary text-white flex-grow-1">
                    <div class="card-body p-4 d-flex flex-column justify-content-center">
                        <div class="d-flex align-items-center gap-3 mb-2">
                            <div class="bg-white bg-opacity-25 p-2 rounded"><i class="bi bi-wallet2 fs-3"></i></div>
                        ...
                        <small class="opacity-75 mt-2 border-top pt-2 border-white border-opacity-25">Total dari pesanan lunas</small>
                    </div>
                </div>
                <div class="row g-3">
                    <div class="col-6">
                ...
                <table class="table table-hover align-middle mb-0">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th>Invoice</th>
                            <th>Bukti Bayar</th> <th>Info Pengiriman</th>
                            <th>Total</th>
                            <th>Status</th>
                        </tr>
                    </thead>
                            ...
                            <td class="ps-4 text-nowrap">
                                ...
                                @if($order->status == 'pending' || $order->status == 'waiting_confirmation')
                                    <form action="{{ route('admin.order.confirm', $order->id) }}" method="POST" class="d-inline action-form">
                                        @csrf
                                        <button type="button" class="btn btn-sm btn-success fw-bold" onclick="confirmPayment(this)" title="Konfirmasi Pembayaran">
                                ...
                                @elseif($order->status == 'shipped')
                                    <span class="badge bg-info text-dark border border-info"><i class="bi bi-truck"></i> OTW</span>
                            ...
                            <td>
                                @if($order->payment_proof)
                                    <a href="{{ asset('storage/'.$order->payment_proof) }}" target="_blank">
                                        <img src="{{ asset('storage/'.$order->payment_proof) }}" width="50" height="50" class="rounded border shadow-sm object-fit-cover">
                                    </a>
                                @else
...
<script>
    ...
    function confirmPayment(button) {
        Swal.fire({
            ...
            text: "Pastikan bukti bayar valid (jika ada).",
            icon: 'question',
            ...
            if (result.isConfirmed) button.closest('.action-form').submit();
        });
    }
